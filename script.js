...
const addNewCard = function (card) {
   ...
   if (card.type === 'cycling' || card.type === 'jogging')
      html += `   <li class="list-group-item">
                           <i class="fas fa-road"></i>&nbsp; Distance :
                           <span class="badge badge-pill bg-dark bg-opacity-75 ms-3"><span>${card.distance}</span>  Km </span>
                        </li>
                        `;

   if (card.type === 'pushups' || card.type === 'squats')
      html += `   <li class="list-group-item">
...
inputType.addEventListener('change', function () {
   ...
   if (type === 'pushups' || type === 'squats') {
      const html = ` 
            <div class="mb-3 changing-field">
                  <label for="reps" class="form-label">Reps:</label>
                  <input id="reps" type="number" class="form-control" placeholder="in reps" />
            </div>`;
      changingField.innerHTML = html;
   } else {
      const html = ` 
            <div class="mb-3 changing-field">
            <label for="distance" class="form-label">Distance:</label>
            <input id="distance" type="number" class="form-control" placeholder="in kms" />
            </div>`;
      changingField.innerHTML = html;
   }
});
...
WorkoutForm.addEventListener('submit', function (e) {
   e.preventDefault();
   addNewCard(calcCaloriesBurnt(getEnteredData()));

   e.target.querySelector('[type=submit]').setAttribute('data-bs-dismiss', 'modal');
   e.target.querySelector('[type=submit]').click();

   resetForm();

   e.target.querySelector('[type=submit]').setAttribute('data-bs-dismiss', 'none');
